...
public class Concat {
    ...
    public void mergeWav(File file1,File file2, File outputFile)
    {
        int RECORDER_SAMPLERATE= 0;
        try {

            File[] selection=new File[2];
            selection[0]=file1;
            selection[1]=file2;
            DataOutputStream amplifyOutputStream = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(outputFile.getAbsolutePath())));
            DataInputStream[] mergeFilesStream = new DataInputStream[selection.length];
            long[] sizes=new long[selection.length];
            for(int i=0; i<selection.length; i++) {
                File file = selection[i];
                sizes[i] = (file.length()-44)/2;
            }
            for(int i =0; i<selection.length; i++) {
                mergeFilesStream[i] =new DataInputStream(new BufferedInputStream(new FileInputStream(selection[i].getAbsolutePath())));
        ...
        long size =0;
        try {
            FileInputStream fileSize = new FileInputStream(outputFile.getAbsolutePath());
        ...
        try {
            RandomAccessFile rFile = new RandomAccessFile(outputFile.getAbsolutePath(), "rw");
            rFile.seek(0);
            rFile.write(header);
            rFile.close();
        } catch (IOException e) {
